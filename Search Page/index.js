...
function handleSearchButtonClick() {
  ...
  var filterState = $dateInput.value.trim().toUpperCase();

  console.log(`filterState:${filterState}`);

  // Set filteredReports to an array of all addresses whose "state" matches the filter
  var filteredReports = data.filter(function(record) {

    // If true, add the report to the filteredReports, otherwise don't add it to filteredReports
    return (record.name.toUpperCase() === filterState) || (record.Location === filterState) || (record.medals.toString() === filterState);
    // return true;
  });
  console.log(`test1:${filteredReports.length}`);
